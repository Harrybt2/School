...
# # # the list[1][2] will return the distance to get from 1 to 2
# # def greedy_tou(edges: list[list[float]], timer: Timer) -> list[SolutionStats]:
# #     # put inf on the diagonals to begin so my code works
# #     for j in range(0,len(edges)):
# #         edges[j][j] = inf

# #     stats = []
# #     # at a starting point, choose the minimum in the reduced-cost matrix to go to next
# #     # if you hit a dead end, restart with a new starting city
# #     # do this until you run out of time
    
    
# #     # choose a starting point, look at everything it connects to, pick the lowest cost edge, then the next lowest cost
# #     # if you end up at the start before having visited them all start over at a new starting point
# #     # run until you find the best one or time out
# #     # reduced, added_cost = reduce_matrix(my_edges)
# #     # cost = cost + added_cost
# #     complete = False

# #     # while complete != True: # until you've found a valid greedy solution
# #     for starting_points in range(0,len(edges)): # there's as many possible starting points as there are cities
# #         if complete == True:
# #             stats.append(SolutionStats(
# #                 tour=visited,
# #                 score=cost,
...
# #             return stats # if you found a valid solution then get out of this!
# #         cost = 0     
# #         reduced = copy.deepcopy(edges)
# #         search_row = starting_points# choose a starting position
# #         ended = False
# #         visited = [starting_points] # keep track of where you've visited
# #         while ended != True:
# #             for n in range(0,len(reduced)): # go through the column of the next city and make it inf
# #                 reduced[n][search_row] = inf            
            
# #             min_dist = inf
            
# #             for cities in range(0, len(reduced[search_row])): # look at everyhwere you could go from your starting city:
# #                 if reduced[search_row][cities] < min_dist:
# #                     added_cost = reduced[search_row][cities] # when you've found the next city, store its info
# #                     row_of_next = cities
# #                     col_of_next = cities
# #                     min_dist = added_cost
# #             cost = cost + added_cost # add the cost to get to the next city
           

# #             # reduced[row_of_next][search_row] = inf # make the return infinite
# #             search_row = row_of_next
# #             # you can end if you found a solution
# #             visited.append(col_of_next) # update your tour with what you've added
# #             # if search_row == starting_points: # did you make it back to the starting city?
# #             if len(visited) == len(edges): # did you include every city once in your tour?
# #                 if reduced[search_row][starting_points] != inf:
# #                     cost += reduced[search_row][starting_points]
# #                     ended = True # if yes than you found your answer!
# #                     complete = True
# #             # visited.pop() # then make sure its not repeated in your tour
                
# #                 else: # if you didnt include every city in the tour then start over!
# #                     ended = True # start back over with a new starting city

            
                   
# #             # reduced,_ = reduce_matrix(reduced) # now reduce that matrix and repeat
# #     """   
# #     return alist which contains:
# #     tour: list[int]
# #     score: float
# #     time: float
# #     max_queue_size: int # pass in 0 probably for greedy
# #     n_nodes_expanded: int # every option I've looked at ( relevant for all of them)
# #     n_nodes_pruned: int # branch and bound, 0 for greedy and depth-first
# #     n_leaves_covered: int # number of solution found for branch and bound
# #     fraction_leaves_covered: float # leaves covered/(leaves covered + ones we pruned)
# #     """
...
# #     # ))
# #     # return stats
# #     return print('no greedy solution')
# # test_matrix = [[inf, 7, 3, 12],[3, inf, 6, 14],[5,8,inf,6],[9,3,5,inf]]
# # graph = [
# #     [0, 9, inf, 8, inf],
# #     [inf, 0, 4, inf, 2],
# #     [inf, 3, 0, 4, inf],
...
# #     [1, inf, inf, 10, 0]
# # ]
# # # print(greedy_best(graph,timer=Timer()))

# # def better_recursion(edges, discovered, cost, best_cost, search_row, best_tour,start_row):
...
# #                 best_cost , best_tour= better_recursion(edges, discovered, cost, best_cost, city, best_tour, start_row)
# #                 discovered.pop()
...
# #     cost = 0
# #     best_cost, best_tour = better_recursion(edges, discovered, 0,math.inf, start_point, best_tour, start_point )
...
# #     [9, 3, 5, 0],
# #     ]
# # # print(branch_and_bound(graph, timer=Timer()))
...
# #                     child = P[0]+[j]# then make it a child
# #                     child_reduced = copy.deepcopy(reduced_matrix)
# #                     for rows in child[:-1]:
# #                         for items in range(0,len(edges)):
# #                             child_reduced[rows][items] = math.inf# make the row of where you started inf
# #                     for col in child[1:]: # for every city in the tour except the first
# #                         for items in range(0,len(edges)):
# #                             child_reduced[items][col] = math.inf
# #                     for city in range(1,len(child)):
# #                         child_reduced[child[city]][child[city-1]] = math.inf
...
# #                     cost = P[1] + connections + cost_to_reduce # add its cost
# #                     if cost <= bssf[1]: # if its not worse, add it to the stack
# #                         stack.append((child, cost))
# #                         # print('changed bssf')
# #                     # else:
...
# #     # for starting_points in range(0,len(edges)): # there's as many possible starting points as there are cities
# #     #     if complete == True:
# #     #         break # if you found a valid solution then get out of this!
# #     #     search_row = starting_points# choose a starting position
# #     #     ended = False
# #     #     visited = [search_row] # keep track of where you've visited
# #     #     while ended != True:
# #     #         min_dist = inf
...
# #     #                 col_of_next = cities
# #     #                 min_dist = added_cost
# #     #         visited.append(col_of_next) # update your tour with what youo've added
# #     #         cost = cost + added_cost # add the cost to get to the next city
# #     #         # modify the reduced cost matrix to get to your next city
# #     #         for n in range(0,len(reduced[search_row])): # go through the row of the starting city and make them all inf
# #     #             reduced[search_row][n] = inf
# #     #         for n in range(0,len(reduced)): # go through the column of the next city and make it inf
# #     #             reduced[n][col_of_next] = inf
# #     #         reduced[row_of_next][search_row] = inf # make the return infinite
# #     #         search_row = row_of_next
# #     #         # you can end if you found a solution
# #     #         if search_row == starting_points: # did you make it back to the starting city?
...
#     heapq.heappush(stack, ([0],cost_to_reduce, reduced_matrix))  # (cost, tour, reduced_matrix)
...
#                     # then alter the matrix and reduce it again
#                     child = P[0]+[j]# then make it a child
#                     child_reduced = copy.deepcopy(P[2])
...
#                         heapq.heappush(stack, (child, cost, child_reduced))
#                         # print('changed bssf')
#                     # else:
#                     #     cost-= connections
#     stats.append(SolutionStats(
